...
class ServerTests extends Tests {
  ...
  test("Address") {
    ...
    withServer(Server.listen()(app)) { server =>
      contentString(Get(s"http://localhost:${server.port}")) should equal (s"Client: 127.0.0.1")
    }
  }
  ...
  test("Upload", Slow) {
    withServer(Server.listen() {
      ...
      case req @ POST at url"/take/$size" =>
        req.read(_.take(size.toLong).chunks.compile.fold(0)((size,chunk) => size + chunk.size)).map { contentSize =>
